...
export function StudentLoginPage({ onBack, onLoginSuccess, onRegister }: StudentLoginPageProps) {
  ...
  const validate = () => {
    ...
    if (!formData.username.trim()) e.username = 'Student ID required'
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user?.role !== 'student') {
        setErrors({ general: 'This account does not have student privileges.' })
        recaptchaRef.current?.reset(); setCaptchaOk(false)
        return
      }
      localStorage.setItem('currentUser', JSON.stringify({
        username: data.user.username, name: data.user.username, role: 'student'
      }))
      onLoginSuccess()
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <motion.div className="w-full max-w-md relative z-10" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <motion.button onClick={onBack} className="flex items-center gap-2 text-sm text-slate-500 hover:text-amber-600 mb-6 transition-colors" whileHover={{ x: -3 }}>
          <ArrowLeft className="w-4 h-4" /> Back
        </motion.button>
        <div className="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
          <div className="bg-gradient-to-r from-amber-500 to-orange-500 px-8 py-10 text-center relative overflow-hidden">
            <div className="absolute inset-0 opacity-10">
              <svg width="100%" height="100%"><defs><pattern id="ds" width="20" height="20" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1" fill="white"/></pattern></defs><rect width="100%" height="100%" fill="url(#ds)"/></svg>
            </div>
            ...
            <p className="text-sm text-amber-200">Academic Access Login</p>
          </div>
          <div className="p-6 sm:p-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-600">Student ID / Email</label>
                <Input type="text" value={formData.username} onChange={e => onChange('username', e.target.value)} placeholder="Enter student ID or email" className={`h-11 bg-slate-50 border-slate-200 focus:border-amber-400 rounded-xl ${errors.username ? 'border-red-400' : ''}`} disabled={isLoading} />
                {errors.username && <p className="text-xs text-red-500">{errors.username}</p>}
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-600">Password</label>
                <div className="relative">
                  <Input type={showPassword ? 'text' : 'password'} value={formData.password} onChange={e => onChange('password', e.target.value)} placeholder="Enter password" className={`h-11 pr-11 bg-slate-50 border-slate-200 focus:border-amber-400 rounded-xl ${errors.password ? 'border-red-400' : ''}`} disabled={isLoading} />
              ...
              <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}>
                <Button type="submit" className="w-full h-11 bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600 rounded-xl font-semibold shadow-lg shadow-amber-500/25" disabled={isLoading}>
              ...
              <div className="text-center space-y-2">
                <button type="button" className="text-xs text-slate-400 hover:text-amber-600 hover:underline" onClick={() => alert('Contact IT Administrator.')} disabled={isLoading}>Forgot Password?</button>
                {onRegister && (
                  <div><button type="button" className="text-xs font-semibold text-amber-600 hover:underline" onClick={onRegister} disabled={isLoading}>New Student? Register here</button></div>
                )}
              </div>
            </form>
          </div>
        </div>
        <p className="text-center text-xs text-slate-400 mt-6">© {new Date().getFullYear()} Edu-Sync · Secure student access</p>
      </motion.div>
    </div>
  )
}